...
import BalanceBreakdown from "../components/wallets/BalanceBreakdown";
import BalanceHistoryLineChart from "../components/wallets/BalanceHistoryLineChart";
...
function WalletDetails() {
  ...
  const groupedTransactions = useMemo(() => {
    ...
    allTransactions.forEach((transaction) => {
      ...
      if (transaction.type !== "transfer" && !transaction.is_system) {
        const baseAmount =
          transaction.base_currency_amount !== null &&
          transaction.base_currency_amount !== undefined
  ...
  useEffect(() => {
    ...
    const fromWallet = walletsData?.wallets?.find(
      (w) => w.id === parseInt(fromWalletId),
    );
    const toWallet = walletsData?.wallets?.find(
      (w) => w.id === parseInt(toWalletId),
    );
  ...
  useEffect(() => {
    const fetchSubcategories = async () => {
      ...
      try {
        const response = await authenticatedFetch(
          `/api/transactions/subcategories/${category}`,
        );
        if (response.ok) {
  ...
  const maxAllowedAmount = useMemo(() => {
    if (!walletsData?.wallets || walletsData.totalLiquidity === undefined)
      return MAX_AMOUNT;
  ...
  const amountOverflowWarning = useMemo(() => {
    if (
      !amount ||
      !walletsData?.wallets ||
      walletsData.totalLiquidity === undefined
    )
      return null;
  ...
  const filteredCategories = useMemo(() => {
    ...
    const systemTransactionCategories = [
      "Initial Balance",
      "Balance Adjustment",
    ];

    if (transactionType === "income") {
      return referenceData.categories.filter(
        (cat) =>
          cat.type === "income" &&
          !systemTransactionCategories.includes(cat.name),
      );
  ...
  const suggestedTags = useMemo(() => {
    if (
      !category ||
      !referenceData?.categoryTagSuggestions ||
      !referenceData?.tags
    )
      return [];
  ...
  const handleCategoryChange = (newCategory) => {
    ...
    const allowedTagIds =
      referenceData?.categoryTagSuggestions
        .filter(
          (cts) =>
            cts.category_id?.toString() === (newCategory ?? "").toString(),
        )
        .map((cts) => cts.tag_id) || [];

    // Filter current selected tags: keep only system tags allowed in new category + all custom tags
    const filteredSelectedTags = selectedTags.filter((tagId) =>
      allowedTagIds.includes(tagId),
    );
    ...
    const restored = categoryTagsMap[newCategory] || {
      selectedTags: [],
      customTags: [],
    };

    const mergedSelectedTags = Array.from(
      new Set([...filteredSelectedTags, ...restored.selectedTags]),
    );
    const mergedCustomTags = Array.from(
      new Set([...filteredCustomTags, ...restored.customTags]),
    );
  ...
  const hasChanges = useMemo(() => {
    ...
    if (transactionType === "transfer") {
      return (
        ...
        normalize(fromWalletId) !==
          normalize(originalValues.fromWalletId || "") ||
        normalize(toWalletId) !== normalize(originalValues.toWalletId || "") ||
        currentDateString !== originalDateString ||
        normalize(transactionDescription) !==
          normalize(originalValues.description)
      );
    } else {
      return (
        normalize(amount) !== normalize(originalValues.amount) ||
        ...
        normalize(subcategory) !==
          normalize(originalValues.subcategory || "") ||
        normalize(transactionWalletId) !==
          normalize(originalValues.walletId || "") ||
        currentDateString !== originalDateString ||
        normalize(merchant) !== normalize(originalValues.merchant || "") ||
        normalize(counterparty) !==
          normalize(originalValues.counterparty || "") ||
        normalize(transactionDescription) !==
          normalize(originalValues.description) ||
  ...
  const adjustBalanceMutation = useMutation({
    mutationFn: async ({ target_balance, description, date }) => {
      const response = await authenticatedFetch(
        `/api/wallets/${walletId}/adjust-balance`,
        {
          method: "POST",
          body: JSON.stringify({
    ...
    (transaction) => {
      ...
      if (transaction.type === "transfer") {
        ...
        if (transaction.manual_exchange_rate) {
          setManualExchangeRate(
            transaction.exchange_rate_used?.toString() || "",
          );
        } else {
          setManualExchangeRate("");
        }
        ...
        setOriginalValues({
          ...
          fromWalletId:
            transaction.amount < 0
              ...
              : transaction.to_wallet_id?.toString(),
          toWalletId:
            transaction.amount < 0
              ? transaction.to_wallet_id?.toString()
              : transaction.wallet_id?.toString(),
  ...
  return (
    <AppLayout>
      <Stack gap="lg">
            ...
            <Button
              ...
              leftSection={
                <IconEdit
                  size={18}
                  style={
                    wallet.is_archived ? { color: "var(--gray-9)" } : undefined
                  }
                />
              }
        ...
        {wallet && (
          <Group gap="md" align="stretch" wrap="nowrap">
            {/* Wallet Overview */}
            <WalletOverview wallet={wallet} walletsData={walletsData} />

            {/* Balance Breakdown */}
            <BalanceBreakdown wallet={wallet} />
          </Group>
        )}
        ...
        >
            ...
            >
              ...
              <Group gap={4} wrap="nowrap" style={{ minWidth: 0 }}>
                ...
                <Tooltip
                  label={wallet.name}
                  withArrow
                  disabled={wallet.name.length <= 30}
                >
                  <Text
                    size="sm"
                    c="var(--gray-12)"
            ...
            >
              <Stack gap={2}>
                ...
                <Text size="sm" style={{ color: "var(--gray-9)" }}>
                  Transactions are preserved for reference and cannot be
                  modified
                </Text>
              </Stack>
            </Alert>
          ...
          <Box>
            <TransactionList
              ...
              emptyStateSubtitle={
                !wallet?.is_archived
                  ? "Transactions for this wallet will appear once you start using it."
        ...
        {wallet.balance_history &&
          wallet.balance_history.filter((item) => item.balance !== null)
            .length > 1 && (
      ...
      <AdjustBalanceModal
        adjustModalOpened={adjustModalOpened}
        handleCloseAdjustModal={handleCloseAdjustModal}
        adjustBalanceMutation={adjustBalanceMutation}
